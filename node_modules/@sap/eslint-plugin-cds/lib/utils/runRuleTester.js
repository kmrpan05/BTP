...
function testRuleWrapper(rule) {
  ...
  function prepareAndRunRule(context) {
    return {
      Program: (/** @type{import('../types').AST.Program} */ node) => {
        const filePath = context.filename
        _initModelRuleTester(filePath, /** @type {any} */(rule.meta)?.model)
        const createValue = rule.create(context)
        const result = createValue.Program?.(/** @type {any} */(node))
        globalCache.clear()
        return result
      }
    }
 ...
 *
 * @param { import('../types').CDSRuleTestOpts } options RuleTester input options
 */
module.exports = function runRuleTester(options) {
  ...
  if (options.root.startsWith(pluginRootPath)) {
    ...
    rule = testRuleWrapper(/** @type {any} */(rules[path.basename(options.root)]()))
  } else {
  ...
  return tester.run(rulename, /** @type {any} */(rule), /** @type {any} */(testerCases))
}
...
function _getDictFiles(input, filenames) {
  ...
  if (globalCache.has(`dictfiles:${input}`)) {
    dictFiles = /** @type {Record<string, string>} */(globalCache.get(`dictfiles:${input}`))
  } else {
    filenames.forEach(file => {
      dictFiles[file] = globalCache.has(`file:${file}`)
        ? /** @type {string} */(globalCache.get(`file:${file}`))
