...
module.exports = {
  findFuzzy,
  /**
   * @param {import('../types').Definition} definition CSN definition object.
   ...
   * @param {string} value
   * @param {import('../types').CSN.Location} loc
   */
  getReplacementsSuggestions: function (context, value, loc) {
    ...
    const invalid = /** @type {string} */ (matches?.[0])
    const startIndex = lineToReplace.indexOf(invalid)
    const candidates = `['${value}']`
    const suggest = {
      messageId: 'ReplaceItemWith',
      data: { invalid, candidates },
      fix: /** @param {any} fixer */ fixer => fixer.replaceTextRange([startIndex, startIndex + invalid.length + 1], candidates)
    }
    return ({
