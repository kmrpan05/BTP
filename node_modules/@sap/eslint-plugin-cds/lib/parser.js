    ...
    return {
      ...
      services: {
        getParsedCsn: function () {
          ...
          let reflectedModel
          try {
            compiledModel = /** @type {any} */(cds).parse(code, compileOptions)
          } catch {
            // Do nothing
          }
          if (compiledModel) {
            try {
              reflectedModel = /** @type {any} */(cds.linked(compiledModel))
        ...
        getInferredCsn: function () {
          ...
          if (!globalCache.has(`model:${rootPath}`) && rootPath) {
            ...
            if (roots) {
              try {
                compiledModel = /** @type {any} */(cds).load(/** @type {any} */(roots), {
                  cwd: rootPath,
                  sync: true,
                  locations: true,
                  messages
        ...
        getEnvironment: () => /** @type {any} */(globalCache.get('options'))?.[0]?.environment,
        /**
         * @param {any} name
         * @param {any} obj
         * @param {any} model
        ...
        getNode: function (obj) {
          ...
          if (obj?.name) {
            ...
            loc = this.getLocation(name, obj, undefined)
          }
 ...
 *
 * @param {any} code Parse file contents
 * @param {object} [loc]
 * @returns ESLint AST
 */
...
function compileModelFromDict (dictFiles, options) {
  ...
  /** @type {any[]} */
  const messages = []
  const compiledModel = /** @type {any} */(cds).compile(dictFiles, {
    sync: true,
    locations: true,
    messages,
    ...options
