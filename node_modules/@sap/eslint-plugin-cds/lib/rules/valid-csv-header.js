...
module.exports = {
  ...
  create: function (context) {
    ...
    function checkValidHeaders () {
      ...
      try {
        model = /** @type {any} */(cds.compile).for.sql(model, { names: cds.env.sql.names, messages: [] })
      } catch(/** @type {any} */ e) {
      ...
      const [cols] = /** @type {any} */(cds.parse).csv(code)
      const missing = cols.filter(/** @param {any} col */ col => !elements.includes(col.toUpperCase()))
      for (const miss of missing) {
        const index = _findInCode(miss, code)
        const loc = sourcecode.getLocFromIndex(index)
        const candidates = findFuzzy(miss, Object.keys(entity.elements).sort())
        const suggest = candidates.map(/** @param {any} cand */ cand => {
          return {
            messageId: 'ReplaceColumnWith',
            data: { column: miss, candidates: cand },
            fix: /** @param {any} fixer */ fixer => fixer.replaceTextRange([index, index + miss.length], cand)
          }
        })
        context.report({
          messageId: 'InvalidColumn',
          data: { column: miss, candidates: candidates.join(', ') },
