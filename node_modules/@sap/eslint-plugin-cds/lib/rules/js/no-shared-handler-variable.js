...
class NoSharedVariable extends CdsHandlerRule {
  ...
  /** @param {any} node */
    ...
    const type = this.#typeDeclarations.find(({loc}) =>
      ...
      || loc?.end.line === node.loc.start.line && [0,1,2].includes(node.loc.start.column - (loc?.end.column ?? 0))
    )
